...
async def pause(_, message: Message):
    try:
      ...
      await message.reply("هل محادثك متصله بالفعل ?")
      return    
    chat_id = chid
    if (chat_id not in callsmusic.pytgcalls.active_calls) or (
        ...
        await message.reply_text("➤ تم الايقاف موقتآ!")
...
async def resume(_, message: Message):
    try:
      ...
      await message.reply("هل محادثتك متصله بالفعل ?")
      return    
    chat_id = chid
    if (chat_id not in callsmusic.pytgcalls.active_calls) or (
...
async def stop(_, message: Message):
    ...
    if chat_id not in callsmusic.pytgcalls.active_calls:
        ...
        await message.reply_text("♨ انتهئ التغيل!")
...
async def skip(_, message: Message):
    ...
    chat_id = chid
    if chat_id not in callsmusic.pytgcalls.active_calls:
        await message.reply_text("♨ لايوجد شي لتخطية !")
    else:
...
async def admincache(client, message: Message):
    try:
      ...
      await message.reply("هل المكالمة متصله بالفعل ?")
      return
    set(
        chid,
        [
    ...
    await message.reply_text("✔ تم تحديث قائمة التخزين المؤقت للادمنية!")
