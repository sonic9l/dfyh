...
def song(client, message):
    ...
    try:
        ...
        m.edit("✘ لم يتم العثور على الأغنية ، يرجى إرسال اسم أغنية صحيح")
        print(str(e))
        return
    m.edit("⌛ جاري التحميل...")
    try:
...
async def ytmusic(client, message: Message):
    ...
    if is_downloading:
        await message.reply_text(
            "هناك عملية تنزيل أخرى قيد التقدم ، حاول مرة أخرى بعد وقت قصير."
        )
        return
    ...
    pablo = await client.send_message(
        message.chat.id, f"**الحصول على {urlissed} من خوادم youtube ، يرجى الانتظار...**"
    )
    if not urlissed:
        await pablo.edit("حدث خطاء!")
        return
    ...
    capy = f"✨ **اسم المقطع :** __{thum}__ \n💭 **مطلوب من قبل :** __{urlissed}__ \n📣 **القناة :** __{thums}__ \n📌 **الرابط :** [click here]({mo})"
    await client.send_video(
        ...
        progress_args=(
            pablo,
            c_time,
            f"`جارٍ تحميل {urlissed} أغنية من YOUTUBE MUSIC!`",
            file_stark,
        ),
    )
    await pablo.delete()
